...
def load_buckets(bucket_type: str, filepath: str) -> list:
    """Extract bucket names from a file.

    :param bucket_type: The type of bucket to load.
    :param filepath: The path to the file containing the bucket names.
...
def save_output(issues: list, filename: str) -> None:
    """Save output to file.

    :param issues: A list of issues to save.
    :param filename: The name of the file to save the issues to.
    """
...
def init_clients(keys_path: str, service: str) -> list:
    """Initialize clients.

    :param keys_path: The path to the credentials file.
    :param service: The name of the storage service.
        ...
        with open(keys_path, "r") as fkeys:
            ...
            try:
                access_key = re.search(
                    r"access_?key_?id ?[=:] ?([^\s]+)", keys, re.I
                ).group(1)
                secret_key = re.search(
                    r"secret(?:_access_)?key ?[=:] ?([^\s]+)", keys, re.I
                ).group(1)
            except AttributeError:
                raise ValueError(
                    "Credentials are not in the expected format. "
...
def enumerate_keys(bucket, bucket_type: str, wordlist_path: str) -> list:
    """Enumerate bucket keys.

    :param bucket: The bucket to enumerate keys from.
    :param bucket_type: The type of bucket to enumerate.
    :param wordlist_path: The path to the wordlist file.
    :return: A list of bucket keys.
    """
    keys = []
...
def test_s3_bucket(bucket_name: str, clients: list, wordlist_path: str = "") -> str:
    ...
    :param clients: A list of clients to use for testing.
    :param wordlist_path: The path to the wordlist file.
...
def test_az_bucket(bucket_uri: str, _, wordlist_path: str = "") -> str:
    """Test Azure buckets.

    :param bucket_uri: The URI of the bucket to test.
    :param _: A placeholder parameter.
    :param wordlist_path: The path to the wordlist file.
    :return: A string containing the test results.
    """
    issues = []
